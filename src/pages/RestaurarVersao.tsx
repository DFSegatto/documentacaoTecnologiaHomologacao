    ...
    <div className="min-h-screen bg-[#f8f7f4] flex flex-col">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 flex-1">
        <div className="flex items-center gap-2 text-sm text-gray-400 mb-6">
          ...
          <span>/</span>
          <Link to={`/registros/${id}`} className="hover:text-gray-600 transition">Registro</Link>
          <span>/</span>
          <Link to={`/registros/${id}/historico`} className="hover:text-gray-600 transition">Histórico</Link>
          <span>/</span>
          <span className="text-gray-700">Restaurar</span>
        </div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 mb-6 flex gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-amber-800">Atenção antes de restaurar</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden mb-6">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <div>
              <p className="text-xs text-gray-400 mb-1">Versão de {versao ? formatarData(versao.editado_em) : ''}</p>
              <h2 className="text-lg font-semibold text-gray-900">{versao?.titulo}</h2>
            </div>
            <span className="text-xs px-2.5 py-1 bg-blue-50 text-blue-700 rounded-full font-medium">Pré-visualização</span>
          </div>
